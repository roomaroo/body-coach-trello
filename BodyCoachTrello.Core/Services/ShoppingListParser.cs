...
public interface IShoppingListParser
{
    ...
    /// <summary>
    /// Parse a shopping list from text content
    /// </summary>
    /// <param name="content">Text content of the shopping list</param>
    /// <param name="name">Name for the shopping list</param>
    /// <returns>Parsed shopping list</returns>
    Task<ParsedShoppingList> ParseTextAsync(string content, string name);
}
...
public class ShoppingListParser : IShoppingListParser
{
    ...
    public async Task<ParsedShoppingList> ParseFileAsync(FileInfo fileInfo)
    {
        ...
        return await ParseTextAsync(content, fileName);
    }

    public async Task<ParsedShoppingList> ParseTextAsync(string content, string name)
    {
        _logger.LogInformation("Parsing shopping list text: {Name}", name);
